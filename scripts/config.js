const CONFIG = {
 ...
 // ===== НОВОЕ: WalletConnect v2 Project ID =====
    WALLETCONNECT_PROJECT_ID: "db2db542806c7cd734ae01f0a52ab129", // ЗАМЕНИТЕ НА РЕАЛЬНЫЙ Project ID с https://cloud.reown.com
    ...
    // ===== НАСТРОЙКИ TELEGRAM MINIAPP =====
    TELEGRAM_MINIAPP: {
        BOT_USERNAME: "crypto_land_test_bot", // тестовый бот (потом замените)
        APP_NAME: "crypto_land_test", // название MiniApp
        ENABLED: true,
        // ===== НАСТРАИВАЕМЫЙ ТЕКСТ =====
        BOT_CHANGE_NOTE: "Скопируйте выбранный вами формат ссылки и отправьте ее вашему рефералу",
        BOT_CHANGE_NOTE_EN: "Copy the link format you choose and send it to your referral"
    },
    ...
    TRANSLATIONS: {
        ru: {
            ...
            no_transactions: "Нет операций для отображения",
            ...
            trust_desc_desktop: "Расширение для браузера Trust Wallet",
            ...
            connect_via_app: "Через приложение",
            connect_qr: "QR-код",
            other_wallets: "Другие кошельки",
            opening_wallet: "Открываем {wallet}...",
            ...
            // ===== НОВЫЕ ПЕРЕВОДЫ ДЛЯ ПОДКЛЮЧЕНИЯ =====
            choose_wallet: "Выберите кошелек",
            connect_via_app_desc: "Открыть в приложении",
            connect_qr_desc: "Показать QR-код",
            
            // ===== НОВЫЕ КЛЮЧИ ДЛЯ МОДАЛКИ РЕФЕРАЛЬНЫХ ССЫЛОК =====
            referral_links_title: "Реферальные ссылки",
            web_link: "Web-ссылка",
            telegram_link: "Telegram MiniApp",
            web_link_desc: "Для браузера на компьютере или телефоне",
            telegram_link_desc: "Открывается прямо в Telegram MiniApp",
        ...
        en: {
            ...
            trust_desc_desktop: "Trust Wallet browser extension",
            ...
            // ===== NEW TRANSLATIONS FOR CONNECTION =====
            choose_wallet: "Choose wallet",
            connect_via_app_desc: "Open in app",
            connect_qr_desc: "Show QR code",

            
            // ===== NEW KEYS FOR REFERRAL LINKS MODAL =====
            referral_links_title: "Referral Links",
            web_link: "Web Link",
            telegram_link: "Telegram MiniApp",
            web_link_desc: "For browser on computer or phone",
            telegram_link_desc: "Opens directly in Telegram",
            copy_link: "Copy",
            close: "Close",
...
CONFIG.getTelegramMiniAppLink = function(refAddress = '') {
    if (!this.TELEGRAM_MINIAPP.ENABLED || !this.TELEGRAM_MINIAPP.BOT_USERNAME) {
        return null;
    }
    
    const baseUrl = `https://t.me/${this.TELEGRAM_MINIAPP.BOT_USERNAME}/${this.TELEGRAM_MINIAPP.APP_NAME}`;
    if (refAddress) {
        const cleanRef = refAddress.replace('0x', '');
        return `${baseUrl}?startapp=ref_${cleanRef}`;
    }
    return baseUrl;
};

CONFIG.getWebLink = function(refAddress = '') {
